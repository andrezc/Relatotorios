    ...
    begin
      case EsteCadastro of
        ...
        begin
          ...
          VerificaDependencia('VENDAS',         'CD_CLIENTE', 'Vendas/Pedidos/Orçam. ', EsteRegistro, result);
        ...
        begin
          ...
          VerificaDependencia('VENDAS_ITEMS',         'CD_PRODUTO', 'Items de Venda/Orçamentos', EsteRegistro, result);
        ...
        begin
          ...
          VerificaDependencia('VENDAS',     'CD_VENDA', 'Vendas/Pedidos/Orçamentos', EsteRegistro, result);
        end;
        cadFornecedores :
        begin
        ...
        begin
          ...
          VerificaDependencia('VENDAS',           'VENDEDOR',       'Vendas/Pedidos/Orçamentos', EsteRegistro, result);
        end;
        cadPracas :
        begin
...
begin
  ...
  try
    if MessageDlg('Confirma exclusão do(a) '+ CadObjeto +': '+EsteRegistro+'?', mtConfirmation, [mbYes,mbNo], 0) <>  mrYes then
      Exit;

    Dependencias := TemDependencias;
    if not Vazio(Dependencias) then
    begin
      if MessageDlg('Este '+CadObjeto+' faz referências a outros registros conforme descrito abaixo.'+PL+'Deseja realmente excluí-lo?'+PL+PL+Dependencias, mtWarning, [mbYes, mbNo], 0) <> mrYes then Exit;
      if MessageDlg('Esta operação é irreversível.'+PL+'Tem certeza que deseja eliminar este '+CadObjeto+'?', mtWarning, [mbYes, mbNo], 0) <> mrYes then Exit;
    end;
